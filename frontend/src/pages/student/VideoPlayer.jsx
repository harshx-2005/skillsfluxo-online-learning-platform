        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const [videoRes, defaultRes] = await Promise.all([
                    ...
                    api.get('/student/default-videos')
                ]);

                setVideo(videoRes.data);

                // Handle both array (old) and object (new) response structures
                const allDefaults = Array.isArray(defaultRes.data) ? defaultRes.data : (defaultRes.data.videos || []);
                setAllVideos(allDefaults);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="flex justify-between items-center mb-6">
                            <button
                                onClick={() => prevVideo && navigate(`/student/video/${prevVideo.id}`)}
                                disabled={!prevVideo}
                                className={`px-4 py-2 rounded-lg font-medium transition-colors ${prevVideo ? 'bg-gray-800 hover:bg-gray-700 text-white' : 'bg-gray-900 text-gray-600 cursor-not-allowed'}`}
                            >
                                Previous
                            </button>
                            <button
                                onClick={() => nextVideo && navigate(`/student/video/${nextVideo.id}`)}
                                disabled={!nextVideo}
                                className={`px-4 py-2 rounded-lg font-medium transition-colors ${nextVideo ? 'bg-neon-orange hover:bg-orange-600 text-white' : 'bg-gray-900 text-gray-600 cursor-not-allowed'}`}
                            >
                                Next
                            </button>
                        </div>
                        ...
                        <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
                            {relatedVideos.length > 0 ? (
                                relatedVideos.map(vid => (
                                    <div
                                        key={vid.id}
                                        onClick={() => navigate(`/student/video/${vid.id}`)}
