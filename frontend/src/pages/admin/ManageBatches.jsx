    ...
    const fetchData = async () => {
        try {
            ...
            const [batchesRes, coursesRes] = await Promise.all([
                api.get('/adminCourse/batch'),
                api.get('/adminCourse/course')
            ]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                ...
                await api.post('/adminCourse/batch', formData);
                toast.success("Batch created successfully");
            }
            setShowModal(false);
            setFormData({ title: '', course_id: '', start_date: '', end_date: '' });
            setEditingId(null);
            fetchData();
        } catch (error) {
                        ...
                        <div key={batch.id} className="bg-dark-card p-6 rounded-xl border border-gray-800 hover:border-neon-orange transition-all group shadow-lg">
                            <div className="flex justify-between items-start mb-4">
                                <div className="p-3 bg-gray-800 rounded-lg group-hover:bg-gray-700 transition-colors">
                                    <FaLayerGroup className="text-2xl text-neon-orange" />
                                </div>
                            ...
                            <p className="text-neon-orange text-sm font-medium mb-4">
                                {batch.course_name || 'Unknown Course'}
                            </p>
                            <div className="text-xs text-gray-500 space-y-1">
                                <p>Start: {new Date(batch.start_date).toLocaleDateString()}</p>
                                <p>End: {new Date(batch.end_date).toLocaleDateString()}</p>
                            </div>
                        </div>
                    ))}
                </div>
